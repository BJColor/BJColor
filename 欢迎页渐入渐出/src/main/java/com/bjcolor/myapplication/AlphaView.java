...
public class AlphaView extends RelativeLayout implements ViewPager.OnPageChangeListener {
    ...
    public void setPoint(Context context, int press, int normal, int width, int height, int left, int top, int right, int bottom) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i == 0) {
                iv_point.setBackgroundResource(press == 0 ? R.mipmap.splash_dot_press_but : press);
            } else {
                iv_point.setBackgroundResource(normal == 0 ? R.mipmap.splash_dot_normal_but : normal);
            }
    ...
    public void onPageSelected(int position) {
        if (getPointVisible() == VISIBLE) {
            ...
            for (int i = 0; i < length; i++) {
                ivPointArray[position].setBackgroundResource(R.mipmap.splash_dot_press_but);
                if (position != i) {
                    ivPointArray[i].setBackgroundResource(R.mipmap.splash_dot_normal_but);
                }
            }
        }
    }
